...
export default function PaidKeyHistoryPage() {
    const [allRecords, setAllRecords] = useState<PaidRecord[]>([]);
    ...
    // Filter records by selected seller
    const records = allRecords.filter(r => {
        if (!showSellerDropdown || selectedSeller === "self") return true;
        const sellerUsername = sellers.find(s => s._id === selectedSeller)?.username || "";
        const createdByUsername = (r.createdBy as any)?.username || "";
        return createdByUsername === sellerUsername;
    });

    const fmtDateTimeSearch = (d: string | null) => {
        if (!d) return "";
        const dt = new Date(d);
    ...
    const filtered = records.filter(r => {
        ...
        const dateStr = fmtDateTimeSearch(r.approvedAt || r.createdAt).toLowerCase();
        return r.key.toLowerCase().includes(search) ||
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#F4F6F9] text-gray-800">
            <div className="flex items-center justify-between px-5 py-3 bg-white border-b border-gray-200">
                <h1 className="text-xl font-light text-gray-700">Paid Key History</h1>
                <div className="text-sm text-[#337ab7]">Home <span className="text-gray-400 mx-1">/</span> Paid Key History</div>
            </div>

            <div className="p-5 flex-1">
                <div className="bg-white rounded shadow-sm">
                    <div className="p-3 border-b border-gray-100 text-[14px] text-gray-700">Paid Key History</div>
                    <div className="p-4">
                                ...
                                {dropdownOpen && (
                                    <div className="absolute top-full left-0 right-0 z-50 bg-white border border-gray-300 shadow-lg mt-0.5 rounded-b">
                                        ...
                                        <div className="max-h-48 overflow-y-auto">
                                            <div
                                                onClick={() => { setSelectedSeller("self"); setSellerSearch(""); setDropdownOpen(false); setPage(1); }}
                        ...
                        {/* Table — Payment, Message, Date (matching screenshot) */}
                            ...
                            <table className="w-full text-[13px] text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {loading ? (
                                        ...
                                        <tr><td colSpan={3} className="p-10 text-center italic text-gray-400">No paid keys found. Approve a key to see it here.</td></tr>
                                    ) : (
                                        paged.map((r, i) => {
                                            ...
                                            return (
                                                <tr key={r._id} className={i % 2 === 1 ? "bg-[#fcfcfc]" : ""}>
                                                    <td className="p-2.5 border-r border-gray-100 font-bold text-gray-700">2200</td>
                                                    <td className="p-2.5 border-r border-gray-100 text-gray-600">Key Paid - {r.notes || r.key}</td>
                                                    <td className="p-2.5 text-gray-500 text-xs">{fmtDateTime(r.approvedAt || r.createdAt)}</td>
                                                </tr>
                                            );
                                        })
                                    )}
